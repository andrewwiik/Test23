...
<?php } ?>
<form id="setup" method="post" enctype='multipart/form-data' action="setup-install.php?step=1&amp;<?php echo $step_language; ?>" novalidate="novalidate">
	<table class="form-table">
		...
		<tr>
			<th scope="row"><label for="first_name"><?php _e( 'Your First Name' ); ?></label></th>
			<td><input name="first_name" type="text" id="first_name" size="25" value="<?php echo( $first_name ); ?>" />
			<p><?php _e( 'Enter your First Name' ); ?></p></td>
		</tr>
		<tr>
			<th scope="row"><label for="last_name"><?php _e( 'Your Last Name' ); ?></label></th>
			<td><input name="last_name" type="text" id="last_name" size="25" value="<?php echo( $last_name ); ?>" />
			<p><?php _e( 'Enter your Last Name' ); ?></p></td>
		</tr>
		<tr>
			<th scope="row"><label for="profile_image"><?php _e( 'Image for your profile' ); ?></label></th>
			<td><input name="profile_image" type="file" id="profile_image" value="<?php echo( $profile_image ); ?>" />
			<p><?php _e( 'Uplaod a Profile Image' ); ?></p></td>
		</tr>
	</table>
...
<?php
	...
	case 1:
		...
		$admin_email  = isset( $_POST['admin_email'] ) ?trim( dcrm_unslash( $_POST['admin_email'] ) ) : '';
		$first_name  = isset( $_POST['first_name'] ) ?trim( dcrm_unslash( $_POST['first_name'] ) ) : '';
		$last_name  = isset( $_POST['last_name'] ) ?trim( dcrm_unslash( $_POST['last_name'] ) ) : '';
		$profile_image  = isset( $_POST['profile_image'] ) ?trim( dcrm_unslash( $_POST['profile_image'] ) ) : '';
		$img_url = "../manage/img/";
		$profile_image_url  = $img_url . $_FILES['profile_image']['name'];
		...
		if ( $error === false ) {
			...
			dcrm_query("INSERT INTO `".DCRM_CON_PREFIX."Users` (`Username`, `SHA1`, `First`, `Last`, `Image`, `LastLoginTime`, `Power`)
			  VALUES ('". $user_name ."', '" . sha1( dcrm_slash( $admin_password ) ) . "', '". $first_name ."', '". $last_name ."', '". $profile_image_url ."', '0000-00-00 00:00:00', '1')");
			...
			// autofill.inc.php
			if(!move_uploaded_file($_FILES['profile_image']['tmp_name'], '../manage/img/' . $_FILES['profile_image']['name'])){
